...
def test_update_book_by_uid(test_client, fake_book_service, test_book, fake_session):
    update_data = {"title": "Updated Test Title", "page_count": 300}
    response = test_client.patch(f"{books_prefix}/{test_book.uid}", json=update_data)
    book_update_data = BookUpdateModel(**update_data)
    assert fake_book_service.update_book_called_once()
    assert fake_book_service.update_book_called_once_with(
        test_book.uid, book_update_data, fake_session
    )
